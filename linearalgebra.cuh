...
namespace threeshape
{
	...
	{
		CALLABLE inline double sqr_len(double x, double y)
		{
			return x * x + y * y;
		}
		...
		inline Vector3d Matrix4x4d::transform(const Vector3d v) noexcept
		{
			Vector3d result;
			result.X = this->E00 * v.X + this->E01 * v.Y + this->E02 * v.Z + this->E03;
			result.Y = this->E10 * v.X + this->E11 * v.Y + this->E12 * v.Z + this->E13;
			result.Z = this->E20 * v.X + this->E21 * v.Y + this->E22 * v.Z + this->E23;
	...
	namespace datastructures
	{
		...
		using ViD = int const; //vertex id

		//todo: generic
		struct Circle
		{
			CALLABLE Circle(double angle, double solidAngle, Vector3d point, Vector3d solidPoint)
				: angle(angle), solidAngle(solidAngle), point(point), solidPoint(solidPoint)
			{
		...
		struct FacetData
		{
			FacetData(int fIdx, int v1, int v2, int v3) noexcept : Facet(fIdx), V1(v1), V2(v2), V3(v3) {  }
